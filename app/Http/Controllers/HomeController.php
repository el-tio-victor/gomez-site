...
class HomeController extends Controller
{
    ...
    {
         if( auth()->user() == null)
            $articles = Article::where('status','published')->orderBy('id','desc')->simplePaginate(4);
        else
            $articles = Article::where('status','published')->orWhere([ 
		['status','=','edited'],['user_id','=',auth()->user()->id] 
    ...
    public function searchCategory($slug){
      ...
      $category = \App\Category::where('category_slug','=',$slug)->first();
	...
	//dd(\App\Category::findBySlug($slug)->name  );
      ...
      $category_name = \App\Category::findBySlug($slug)->name;
    ...
    public function searchTag($slug){
        ...
        $tags = \App\Tag::where('tag_slug','=',$slug)->first();
	   $tag_name = $tags->name;
